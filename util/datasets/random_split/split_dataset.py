...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Split paired image/mask dataset into train/valid/test"
        )
    )
    parser.add_argument(
        "--images",
        ...
        nargs="+",
        help=(
            "Path(s) to images folder(s) (non-recursive). Multiple "
            "directories are allowed and will be merged."
        ),
    )
    parser.add_argument(
        "--masks",
        ...
        nargs="+",
        help=(
            "Path(s) to masks folder(s) (non-recursive). Multiple "
            "directories are allowed and will be merged."
        ),
    )
    parser.add_argument(
        ...
        required=True,
        help=(
            "Output root path (will create train/valid/test under this)"
        ),
    )
    parser.add_argument(
        "--train",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do not actually copy/move files, only produce the log"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use the second input source (index 1) as a preserved test "
            "set when exactly two inputs are provided."
        ),
    )
    parser.add_argument(
        "--preserve-test-index",
        ...
        default=None,
        help=(
            "Preserve the specified input index as the test set while "
            "splitting the remainder into train/valid."
        ),
    )
    # By default append a run timestamp to avoid clobbering previous outputs.
    parser.add_argument(
        "--no-append-timestamp",
        action="store_false",
        dest="append_timestamp",
        help=(
            "Do not append a run timestamp to the output directory name "
            "(default: append)."
        ),
    )
    ...
    # If enabled, append a run timestamp (yyyymmddHHMM) to the output root
    # directory name only. Example: /path/to/out -> /path/to/out_20251027HHMM
    if append_ts:
        run_ts = datetime.now().strftime('%Y%m%d%H%M')
        # Normalise path (remove trailing slash) then append _timestamp to the
        # final component.
    ...
    # Detect duplicates across sources: require exactly one image and one mask
    # per basename.
    ...
    if dup_images or dup_masks:
        print(
            "Duplicate basenames detected across input directories. "
            "This tool expects unique basenames across image and mask dirs."
        )
        if dup_images:
        ...
        print(
            "Please ensure basenames are unique or adjust the script to "
            "handle duplicates."
        )
        sys.exit(1)

    # Create pairs selecting the single entry for each basename:
    # (basename, image_filename, mask_filename, image_dir, mask_dir)
    pairs_all = []
    ...
    if args.preserve_original_test:
        ...
        if len(images_dirs) == 2 and len(masks_dirs) == 2:
            ...
            print(
                "--preserve-original-test requires exactly two input "
                "sources (images and masks)."
            )
            sys.exit(1)
    ...
    if preserve_index is not None:
        ...
        if (
            preserve_index < 0
            or preserve_index >= len(images_dirs)
            or preserve_index >= len(masks_dirs)
        ):
            print(
                f"preserve-test-index {preserve_index} out of range for "
                "provided sources"
            )
            sys.exit(1)
        # Filter pairs_all to find those whose source dirs match the preserve
        # index dirs.
        ...
        for p in pairs_all:
            ...
            img_match = os.path.abspath(img_dir) == os.path.abspath(
                preserve_img_dir
            )
            mask_match = os.path.abspath(mask_dir) == os.path.abspath(
                preserve_mask_dir
            )
    ...
    # Entries look like: (basename, img_fname, mask_fname, img_dir, mask_dir,
    # split)
    final_items = []
    ...
    if preserve_idx is not None:
        ...
        if (
            preserve_idx < 0
            or preserve_idx >= len(images_dirs)
            or preserve_idx >= len(masks_dirs)
        ):
            print(
                f"--preserve-test-index {preserve_idx} is out of range for "
                "the provided input directories"
            )
            sys.exit(1)
        ...
        if n_rem > 0:
            ...
            if tv_sum <= 0:
                print(
                    "train+valid ratios must be > 0 when preserving a test "
                    "source"
                )
                sys.exit(1)
    ...
    # Write CSV and perform copy/move (or dry-run) using the final assignments
