...
## Dataset split helper (util/datasets/split_dataset.py)

A small helper script is provided to split paired image/mask datasets into
train/valid/test subsets. Important behavior changes:

- By default the script appends a run timestamp to destination filenames in
	the format `_yyyymmddHHMM` (for example `image_20251027XXXX.png`). This
	prevents accidental overwrites when you run multiple splits into the same
	output folder.
- If you prefer to keep original filenames, run with `--no-append-timestamp`.
- To remove an existing output folder before writing (force clean output),
	use `--overwrite`.

Example usage:

```bash
python3 util/datasets/split_dataset.py \
...
- `--overwrite` : remove existing output folder before writing (use with care)
- `--no-append-timestamp` : disable appending timestamp to destination filenames

The script produces a CSV log and a human-readable summary under the output
folder. Use `--dry-run` to preview actions without copying/moving files.
